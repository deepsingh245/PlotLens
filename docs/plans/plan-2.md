...
**Re-verified during Phase 3 (2026-08-11/17): the bug was still present** — a headless-Chromium check on `/projects/mock-1` still measured `.maplibregl-canvas`'s bounding box at `height: 300` after all three fixes above. Deferred at the user's direction rather than debugging live at that point.

**Fixed (2026-08-18) — real root cause found.** The `absolute inset-0` fix from 2026-08-11 was necessary but not sufficient: MapLibre's `_setupContainer()` adds a `maplibregl-map` class to the exact container element `MapCanvas.tsx` owns, and `maplibre-gl.css`'s `.maplibregl-map { position: relative }` rule has the same specificity as Tailwind's `.absolute` utility — whichever stylesheet the build happened to place later in the cascade won, and it silently canceled the `absolute` positioning `inset-0` depends on. `clientHeight` was therefore genuinely `0` at all times (not a one-off race), so MapLibre's own `clientHeight || 300` fallback in `_containerDimensions()` kicked in permanently — no `ResizeObserver` could ever fix it, because the container never actually had a nonzero size to observe. Fix: `MapCanvas.tsx`'s container now sets `position: absolute` via an inline `style` prop instead of a `className` — an inline style always outranks a class-selector rule in the CSS cascade, so it can't lose to `maplibre-gl.css` regardless of stylesheet load order. Verified via DOM inspection (canvas now measures its real container height, e.g. 686px, not the 300px fallback) and a screenshot showing actual OpenStreetMap tiles rendering for the first time. This also unblocks the same-risk callouts in [plan-3.md](plan-3.md)/[plan-5.md](plan-5.md)/[plan-7.md](plan-7.md) below — `OverlayControlBar`, the layers panel, and the measurement label all depend on the map container's real rendered size and can now be trusted.
