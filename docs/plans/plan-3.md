...
**Carried-over risk, now resolved:** the Phase 2 map-sizing bug (canvas container stuck at `height: 300`) directly affected `OverlayControlBar`'s screen-space positioning and the corner-drag math, both of which depend on the map container's real rendered size — see [plan-2.md](plan-2.md)'s 2026-08-18 update for the actual root cause (a CSS cascade conflict with `maplibre-gl.css`, not a timing race) and fix. `OverlayControlBar`'s positioning and corner-drag math can now be trusted to reflect the map's real size; full interactive corner-dragging verification is still worth doing explicitly, but is no longer blocked on this bug.
